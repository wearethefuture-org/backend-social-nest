...
export class UsersService {
  ...
  public async update(
    ...
    if (isAuthUserId || isSuperAdmin) {
      ...
      throw new HttpException(
        'The user does not have access to change user data with this ID',
  ...
  public async getDataAnalytic(filterDto: AnalyticsFilterDto): Promise<object> {
    ...
    if (filterDto.name === AnalyticsFilterNameEnum.users) {
      ...
      analytics.countSelect = await this.userRepository.getCountSelectUsers(
        filterDto,
      );
    ...
    analytics.countSelect = await this.chatsRepository.getCountSelectChats(
      filterDto,
    );
  ...
  public async getCounters(id: number): Promise<object> {
    ...
    const countMessages = await this.messagesRepository.getCountMessagesUser(
      id,
    );
